...
function parseUplink(device, payload) {
    ...
    if (decoded.o3 != null) {
        ...
        if (sensor2 != null)
            sensor2.updatePpmConcentrationSensorStatus(decoded.o3);
    };
    ...
    if (decoded.co != null) {
        ...
        if (sensor3 != null)
            sensor3.updatePpmConcentrationSensorStatus(decoded.co);
    };
    // Store NO
    if (decoded.no != null) {
        ...
        if (sensor4 != null)
            sensor4.updatePpmConcentrationSensorStatus(decoded.no);
    };
...
function Decoder(bytes, fport) {
    ...
    if (fport === 6) { // then its ReportDataCmd
        if (bytes[2] === 0x00) { // version report
            ...
            return decoded;
        } else if ((bytes[0] === 0x01) && (bytes[1] === 0x57) && (bytes[2] === 0x05)) { // device type 57 (R718PA3)
            decoded.devicetype = "R718PA3";
            decoded.battery = bytes[3] / 10;
            decoded.o3 = ((bytes[4] << 8) | bytes[5]) / 10;
            decoded.co = ((bytes[6] << 8) | bytes[7]) / 10;
            decoded.no = ((bytes[8] << 8) | bytes[9]) / 10;
            return decoded;
        }
